...
class NewsListIntegrationTest {
    ...
    @Test
    fun `complete flow from usecase to viewmodel should work correctly`() = runTest(testDispatcher) {
        // Given
    ...
    @Test
    fun `error handling should propagate correctly through layers`() = runTest(testDispatcher) {
        // Given
        val errorMessage = "Network connection failed"
        coEvery { getPopularNewsUseCase.invoke(any()) } returns flow { 
    ...
    @Test
    fun `different days periods should be handled correctly end-to-end`() = runTest(testDispatcher) {
        // Given
        val daysPeriods = listOf(1, 7, 30)
        val responses = daysPeriods.mapIndexed { index, days ->
    ...
    @Test
    fun `empty results should not change state from loading`() = runTest(testDispatcher) {
        // Given
    ...
    @Test
    fun `multiple rapid fetch events should be handled gracefully`() = runTest(testDispatcher) {
        // Given
