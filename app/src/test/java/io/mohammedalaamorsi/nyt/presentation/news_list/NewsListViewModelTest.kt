...
class NewsListViewModelTest {
    ...
    private val mockNewsItems = listOf(
        Result(
    ...
    @Test
    fun `initial state should be Loading and then fetch news automatically`() = runTest {
        // Given
        val expectedResponse = MostPopularApiResponse(
            copyright = "Copyright",
            numResults = 2,
    ...
    @Test
    fun `onEvent FetchNews should update state to Result when success`() = runTest {
        // Given
        val daysPeriod = 30
        val expectedResponse = MostPopularApiResponse(
            copyright = "Copyright",
    ...
    @Test
    fun `onEvent FetchNews should update state to Error when exception occurs`() = runTest {
        // Given
        val errorMessage = "Network error"
        // Mock the initial call from init block to return a flow that throws
        coEvery { getPopularNewsUseCase.invoke(7) } returns flow { 
    ...
    @Test
    fun `should not update state when response has empty results`() = runTest {
        // Given
        val daysPeriod = 7
        val emptyResponse = MostPopularApiResponse(
            copyright = "Copyright",
            numResults = 0,
            results = emptyList(),
            status = "OK"
    ...
    @Test
    fun `effects should remain empty when no errors occur`() = runTest {
        // Given
        val expectedResponse = MostPopularApiResponse(
            copyright = "Copyright",
            numResults = 1,
    ...
    @Test
    fun `multiple fetch events should work correctly`() = runTest {
        // Given
