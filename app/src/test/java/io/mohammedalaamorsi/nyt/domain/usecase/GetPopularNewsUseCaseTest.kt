...
class GetPopularNewsUseCaseTest {
    ...
    @Test
    fun `invoke should return flow from repository with correct dispatcher`() = runTest {
        // Given
        val daysPeriod = 7
    ...
    @Test
    fun `invoke should work with different days periods`() = runTest {
        // Given
        val daysPeriods = listOf(1, 7, 30)
        val responses = daysPeriods.map { days ->
